...
//내적 구하는 함수
float MathClass::dotProduct(float x0, float y0, float x1, float y1)
{
	return (x0 * x1) + (y0 * y1);
}

//원점에서 점까지의 거리 백터값을 구하는 함수
float MathClass::vectorLength(float x, float y)
{
	return sqrt((x * x) + (y * y));
}

//두점의 거리를 구하는 함수
float MathClass::distanceBetweenTwoPoint(float x0, float y0, float x1, float y1)
{
...
//두개의 원의 충돌 검사
bool MathClass::checkCircleCollision(float _distance, float _radius1, float _radius2)
{
...
float MathClass::angleBetweenTwoPoint(float dot, float vectorLength1,float vectorLength2)
{
