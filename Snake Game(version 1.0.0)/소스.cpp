...
int main(void)
{
	...
	while (1)
	{
		if (toggleKey == true) // w,s,a,d 키가 눌렸을때 루틴 진입
		{
			...
			{
				if (!(bodyX[i] == 0) || !(bodyY[i] == 0)) // 초기화 값이 아닌 플레이어가 움직인 값이 들어가 있다면
				{
...
//			DEBUG_FUNCTION;
		...
		//마지막 몸통 자리 빈칸으로 지우기
		if (!(bodyX[BODY_MAX_LENGTH - 1] == 0) || !(bodyY[BODY_MAX_LENGTH]))
		{
		...
		else if (GameMap[playerNextY][playerNextX] == 2) // 아이템 획득
		{
			...
			GameMap[playerY][playerX] = 0; // 기존 아이템 위치를 빈공간으로
...
void mapDraw(void)
{
	for (int heightMap = 0; heightMap < HEIGHT_MAP_SIZE; heightMap++)
	{
		for (int widthMap = 0; widthMap < WIDTH_MAP_SIZE; widthMap++)
		{
			gotoxy(widthMap * 2, heightMap);
